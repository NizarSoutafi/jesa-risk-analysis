...
import {
  ...
  Settings as SettingsIcon, LockReset as LockResetIcon
...
const Dashboard = () => {
  // Core operational state managed here
  ...
  const [isLoading, setIsLoading] = useState(false); // General loading for major ops
  const [isProcessingFile, setIsProcessingFile] = useState(false);
  const [isAnalyzing, setIsAnalyzing] = useState(false);

  // Sidebar Risk Settings State
  ...
  // Profile Menu & Settings Dialog state
  ...
  const handlePasswordChange = async () => {
    // ... (password change logic remains the same as previous version)
    setPasswordChangeError(''); setPasswordChangeSuccess('');
  ...
  const handleAddRisk = () => {
    // ... (add risk logic remains the same as previous version)
  ...
  // --- Handlers for DashboardHome to use via Context ---
  ...
  const handleUploadContext = async () => {
    if (!file) { setError('Please select an XER file to upload from Dashboard context.'); return; }
  ...
  const handleRunAnalysisContext = async () => {
    if (!parsedData || !scheduleAnalysis) { setError('Please upload and process an XER file first (from Dashboard context).'); return; }
    ...
    try {
      ...
      const response = await axios.post('http://localhost:8000/api/run-analysis', {
        task_data: parsedData.TASK || [], taskpred_data: parsedData.TASKPRED || [],
        n_simulations: nSimulations, buffer_days: bufferDays, user_risks: userRisks, // These come from Dashboard state
      }, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const contextValue = {
    file, setFile: handleFileChangeContext, // Pass the wrapped handler
    parsedData, scheduleAnalysis, monteCarloResults,
    error, successMessage, isLoading, isProcessingFile, isAnalyzing,
    setError, setSuccessMessage,
    handleUpload: handleUploadContext, // Pass the wrapped handler
    handleRunAnalysis: handleRunAnalysisContext, // Pass the wrapped handler
    // Risk settings can also be part of context if needed by DashboardHome directly,
    // but for now, handleRunAnalysis gets them from Dashboard's state.
  };
  ...
  const sidebarContent = (
    <Box sx={{ p: 1.5, display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <List dense>
        <ListItem disablePadding> <ListItemButton onClick={() => navigate("/dashboard")} sx={{py: 1, borderRadius: 1.5}}> <Assessment sx={{ mr: 1.5 }} /> <ListItemText primary="Overview" /> </ListItemButton> </ListItem>
      ...
      <Box sx={{ pt: 1 }}>
        ...
        <Collapse in={isRiskSettingsOpen} timeout="auto" unmountOnExit sx={{ maxHeight: 'calc(100vh - 520px)', overflowY: 'auto', pr: 0.5, pl: 0.5, mt: 1, mr:-0.5, ml:-0.5 }}>
          <List dense sx={{p:1, bgcolor: 'rgba(0,0,0,0.05)', borderRadius:1.5}}>
            ...
            <ListItem sx={{ px: 0.5, py: 0.8 }}>
              <FormControl fullWidth variant="outlined" size="small"> <InputLabel id="affected-task-select-label">Affected Task</InputLabel>
                <Select labelId="affected-task-select-label" value={riskTask} label="Affected Task" onChange={(e) => setRiskTask(e.target.value)} MenuProps={{ PaperProps: { sx: { maxHeight: 200 } } }}>
                  ...
                  {/* This needs access to parsedData.TASK. If parsedData is in context, DashboardHome can populate context, or this part needs context too. */}
                  {/* For now, assuming parsedData is in context or we need a different source for tasks here */}
                  {contextValue.parsedData?.TASK?.map((task) => (<MenuItem key={task.task_id} value={task.task_name}>{task.task_id} - {task.task_name}</MenuItem>))}
                </Select>
              </FormControl>
            </ListItem>
            <ListItem sx={{ px: 0.5, py: 1 }}> <Button variant="contained" color="secondary" onClick={handleAddRisk} startIcon={<AddCircleOutline />} fullWidth> Add Risk </Button> </ListItem>
            ...
            {userRisks.length > 0 && ( <ListItem sx={{ display: 'block', px: 0.5, mt: 1 }}> <Typography variant="caption" component="div">Added Risks ({userRisks.length}):</Typography> <List dense sx={{ maxHeight: 80, overflow: 'auto', bgcolor: 'rgba(0,0,0,0.1)', borderRadius: 1, p:0.5 }}> {userRisks.map((risk, index) => ( <ListItemText key={index} primary={`${risk["Risk ID"]}: ${risk.Description.substring(0,25)}... (${risk.Probability * 100}%)`} primaryTypographyProps={{fontSize: '0.75rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}}/> ))} </List> </ListItem> )}
          </List>
        </Collapse>
      </Box>
    </Box>
    ...
    <DashboardContext.Provider value={contextValue}>
      <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
        ...
        <Box component="main" sx={{ flexGrow: 1, p: { xs: 2, sm: 3 }, width: `calc(100% - ${drawerWidth}px)`}}>
          <Outlet /> {/* Child routes (DashboardHome, UploadHistory) will render here */}
        </Box>
        {/* Settings Dialog */}
